...
class RestVtep(app_manager.RyuApp):
    ...
    def _peer_down_handler(self, remote_ip, remote_as):
        neighbor = self.speaker.get_links.get(remote_ip, None)
        if neighbor is None:
    ...
    def del_speaker(self):
        ...
        for address in list(self.speaker.get_links.keys()):
    ...
    def add_neighbor(self, address, remote_as):
        ...
        neighbor = EvpnNeighbor(
            address=address,
            remote_as=remote_as)
        self.speaker.get_links[address] = neighbor
    ...
    def get_neighbors(self, address=None):
        ...
        if address is not None:
            neighbor = self.speaker.get_links.get(address, None)
            if neighbor is None:
                raise NeighborNotFound(address=address)
            return {address: neighbor.to_jsondict()}

        neighbors = {}
        for address, neighbor in self.speaker.get_links.items():
    ...
    def del_neighbor(self, address):
        ...
        neighbor = self.speaker.get_links.pop(address)
    ...
    def del_network(self, vni):
        ...
        for address in self.speaker.get_links:
